...
HRESULT CubeMapApp::loadLUTS(char* fileName, LPCSTR shaderTextureName, int xRes, int yRes, ID3D10Device* pd3dDevice)
{
    HRESULT hr = S_OK;
    ...
    {   
		...
		ID3D10EffectShaderResourceVariable* textureRVar = fx::CubeMapFX->GetVariableByName( shaderTextureName )->AsShaderResource();
...
void CubeMapApp::initApp()
{
	...
	mBallMapRV        = GetTextureMgr().createTex(L"bricks.dds");
    ...
    if(loadLUTS("G0_pi_2_64_data.csv","Gtable",64,64, md3dDevice) == S_FALSE)
        loadLUTS("G0_pi_2_64_data.csv","Gtable",64,64, md3dDevice); 
    if(loadLUTS("G20_pi_2_64_data.csv","G_20table",64,64, md3dDevice) == S_FALSE)
        loadLUTS("G20_pi_2_64_data.csv","G_20table",64,64, md3dDevice); 
}
...
void CubeMapApp::updateScene(float dt)
{
	...
	if(GetAsyncKeyState('A') & 0x8000)	GetCamera().strafe(-1300.0f*dt);
	if(GetAsyncKeyState('D') & 0x8000)	GetCamera().strafe(+1300.0f*dt);
	if(GetAsyncKeyState('W') & 0x8000)	GetCamera().walk(+1300.0f*dt);
	if(GetAsyncKeyState('S') & 0x8000)	GetCamera().walk(-1300.0f*dt);
